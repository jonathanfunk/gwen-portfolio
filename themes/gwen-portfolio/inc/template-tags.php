 ...
 */
 ...
 function gwen_entry_footer() {
 	...
 	if ( 'post' === get_post_type() ) {
 		...
 		if ( $categories_list && gwen_categorized_blog() ) {
  ...
  * @return bool
  */
 function gwen_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'gwen_categories' ) ) ) {
 		...
 		set_transient( 'gwen_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so gwen_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so gwen_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in gwen_categorized_blog.
  */
 function gwen_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'gwen_categories' );
 }
 add_action( 'edit_category', 'gwen_category_transient_flusher' );
 add_action( 'save_post',     'gwen_category_transient_flusher' );
  ...
  */
 function gwen_comment_list( $comment, $args, $depth ) {
  ...
  */
 function gwen_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
